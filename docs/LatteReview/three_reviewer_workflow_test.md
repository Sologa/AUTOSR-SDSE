...
- 輸入資料：`test_artifacts/metadata_harvest/speech_language_models_per_query/arxiv_per_query_metadata.json`
  - 測試會過濾掉標題含有 `survey` 的條目，避免在撰寫 survey 時引用其他 survey；改取前 `LATTE_REVIEW_TOP_K` 筆主要研究
...
- 參數化選項：
  - `LATTE_REVIEW_TOP_K`（環境變數，預設 `10`）可限制最多處理的條目數，以避免一次建立過多 reviewer 任務
  - 測試結果會輸出至 `test_artifacts/metadata_harvest/speech_language_models_per_query/latte_review_results_top<X>.json`，其中 `X` 為實際審查的資料筆數
...
export LATTE_REVIEW_TOP_K=10  # 預設即為 10，可視需求調整
