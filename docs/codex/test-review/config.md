...
這是專案層設定，不會影響使用者全域設定；詳細來源與說明見 `docs/codex/test-review/gemini_web_search_control.md`。若需一次性允許 Web Search，請加 `--gemini-allow-web-search`。

5. 需要調整 approval/automation (例如 `--full-auto`) 時，請透過 `docs/codex/test-review/run_*` 的 `--codex-extra-arg` 參數在 codex exec 之前插入 flag，執行後結果與 manifest 會記錄實際命令。`web_search` 工具預設被關閉；若某次要允許，可加上 `--allow-web-search`。
