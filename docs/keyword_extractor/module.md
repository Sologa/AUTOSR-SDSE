...
Purpose
- Extract anchor terms, categorized search terms, and metadata-aligned paper summaries from one or more survey PDFs using the LLM provider layer.
...
- Python: `src/utils/keyword_extractor.py`
  ...
  - `build_generate_instructions(params, resolved_anchor_variants=None, metadata_block=None)` and `build_aggregate_instructions(partials, max_queries=50, topic=None, anchor_variants=None, metadata_block=None, allow_additional_categories=True, category_target=None)` helpers.
...
- Final JSON includes `papers`: an ordered list containing the canonical metadata plus `detected_keywords` extracted from the PDFs.
- `_validate_output_against_metadata` ensures `papers` align with the metadata and titles/abstracts remain exact copies.
...
- JSON scaffolding in the prompt remains neutral (`<category>` placeholders) so custom taxonomies can be inferred without further string replacements. The schema exposes `papers[*].source_id/title/abstract/year/url` for downstream use.
...
Usage Logging
- If omitted, logs default to `test_artifacts/keyword_extractor_live/keyword_extractor_usage_<timestamp>.json`; override `usage_log_path` to control location.
