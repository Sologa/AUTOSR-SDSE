...
- **共通前置作業**：先以 `_collect_paper_metadata` 下載 arXiv 中繼資料、使用 `get_title_abstract_profile` 建立 LatteReview 評審設定、透過 `_resolved_anchor_variants` 推導錨點詞，並在 `test_artifacts/llm/keyword_extractor_usage_*.json` 紀錄 LLM 用量。

## 提示詞來源與佔位符
- **生成提示**：必須存在於 `resources/LLM/prompts/keyword_extractor/generate_search_terms.md`。程式會依 `ExtractParams` 替換 `<<max_queries>>`、`<<category_list>>`、`<<anchor_guidance>>` 等佔位符，並插入動態生成的論文中繼資料與評審設定區塊。
- **聚合提示**：必須存在於 `resources/LLM/prompts/keyword_extractor/aggregate_terms.md`，同樣替換 `<<topic_hint>>`、`<<anchor_policy>>`、`<<partial_json_list>>` 等內容，並附上統一的中繼資料與評審設定。若檔案缺失會直接拋出 `FileNotFoundError`。
...
- 目標：產出僅含 JSON 的輸出，內容需包含錨點詞、分類後的檢索詞、逐字複製的評審設定（`reviewer_profile`）與論文中繼資料。所有欄位都必須以 PDF 與提供的中繼資料為依據。
- 限制：
  - 僅能使用上傳的 PDF 或附加的中繼資料與評審設定區塊中的資訊。
...
- 流程：
  ...
  6) 依附加的評審設定逐字填入 `reviewer_profile` 物件；不得增刪欄位或修改內容。
- 執行時覆寫（目前請求）：
...
- 輸出格式（嚴格 JSON，不得包含額外文字）：
{
  ...
  "reviewer_profile": {
    "review_topic": "<評審設定逐字複製>",
    "inclusion_criteria": "<評審設定逐字複製>",
    "exclusion_criteria": "<評審設定逐字複製>",
    "backstory": "<評審設定逐字複製>",
    "reasoning": "<評審設定逐字複製>",
    "additional_context": "<評審設定逐字複製>",
    "examples": ["<逐字複製每個範例；若無則為 []>"],
    "provider": {
      "model": "<複製 provider model，若無則為 null>",
      "model_args": {"<逐字複製每個參數>": "…"}
    }
  },
  "papers": [
...
```json
{
  ...
  "reviewer_profile": {
    "review_topic": "示例審查者：針對對話摘要研究",
    "inclusion_criteria": "納入 2019 年後發表、聚焦抽象式對話摘要的同儕審查研究。",
    "exclusion_criteria": "排除非英語或僅聚焦抽取式摘要的研究。",
    "backstory": "專門研究對話摘要技術與資料集的審查者",
    "reasoning": "brief",
    "additional_context": "請標註每篇論文主要對應的 CADS challenge（language、structure、comprehension、speaker、salience、factuality）。",
    "examples": [
      "Include: 分析對話摘要資料集與評量指標的綜述論文。",
      "Exclude: 僅處理多模態摘要、未提供純文字對話結果的工作。"
    ],
    "provider": {
      "model": null,
      "model_args": {}
...
- 流程：
  ...
  5) 將評審設定逐字複製到 `reviewer_profile`；不得增刪欄位。
- 提供的中繼資料（逐字複製，並保留順序）：
<<paper_metadata_block>>
- 評審設定（逐字複製到 `reviewer_profile`）：
<<reviewer_profile_block>>
