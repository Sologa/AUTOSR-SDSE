...
- 標準流程：**建立分支 → 本地檢查（ruff/mypy/pytest）→ 更新 docs/logs → commit → push 分支 → 建立 PR**。
- **Commit 節奏**：一個邏輯單元一個 commit；訊息遵循 Conventional Commits（例：`feat(core): add scoring`）。
- **Agent 自動提交限制**：
  - 僅允許對 **`docs/**` 與 **`logs/**`** 的純內容變更自動 **commit & push 到 feature 分支**，並自動建立 PR；
  - 任何涉及 **`src/**` 或 **`tests/**`** 的改動，**不得**自動提交，務必走 PR 審查；
  - **嚴禁**任何情況下直推 `main`/`master`。
...
- PR 標題：`[模組] 簡述`；描述包含動機與測試要點 ✅

> CI 會比照上述條件執行；不合規 PR 將被拒絕。
